...
Die Retrospektive dient als Stimmungsbarometer im Team. Ungefähr im dritten Monat des Projekts haben wir, angeleitet durch Herrn Schmidt, diesem Thema ein Treffen gewidment. Hierbei hatte jedes Teammitglied die Möglichkeit Probleme und Schwierigkeiten auf zwischenmenschlicher Ebenen anzusprechen. Die so gesammelten Themen wurden dann von allen Teammitgliedern priorisiert und über die wichtigsten drei Punkte wurde im Detail gesprochen. Besonders wichtig war dabei, dass zum Ende, für alle genauer besprochenen Themen, das weitere Vorgehen abgestimmt wurde, damit solche Probleme in Zukunft nicht mehr auftreten würden.\\
Die Retrospektive war für das Team von großer Wichtigkeit. Die meisten Teammitlieder hatten ähnliche Sorgen und durch diesen Rahmen hatten wir die Möglichkeit diese erfolgreich auszuräumen. Nach diesem Treffen haben sich die dort besprochenen Probleme entscheidend entschärft.
\subsubsection{User Stories}
\label{chap:userstories}
\textit{User Stories} oder Tickets sind die kleinste Einheit der Arbeitspakete eines Projekts. Sie werden aus der Sicht des Produktanwenders geschrieben und werden mit einer vorgegebenen Form formuliert. Ein Beispiel aus unserem Projekt ist folgendes. 'As a player I want to see the values of the bars in the bar chart in percent in the game screen.' Siehe Abbildung \ref{fig:userstory}.\\
...
Ein Styleguide wurde erstellt, in welchem jede Farbe und Schriftgröße so getestet wurde, dass sie den Werten des AA Standards der W3C Verordnung entspricht. Außerdem nahmen wir noch andere Richtlinen wie Clean Coding Regeln in unseren Styleguide mit auf. Mehr dazu in \autoref{chap: Anlagen} .
...
\caption{\label{fig:quickDraw}Quick, Draw! Dataset}
\end{figure}
...
\caption{\label{fig:networkArchitektur}Architektur des neuronalen Netzwerkes}
\end{figure}
...
\section{Fazit}
Zurück blickend auf unsere, am Anfang des Projektes festgelegte, Produktvision, sind wir uns sicher, dass wir unser Ziel erreicht haben. Innerhalb der letzten viereinhalb Monaten haben wir ein Spiel entwickelt, das auf den, auf dem deutschen Markt am verbreitesten, Smartphones spielbar ist. Dass das Spiel bei meheren Generationen gut ankommt, konnten wir live auf der Showtime testen und beobachten.\\
Während des Projektes hatten wir einige Hürden, die wir bewältigen mussten. Im Folgenden wollen wir auf drei etwas näher eingehen.
\subsection{Einigung auf ein Spielekonzept}
Am Anfang des Projektes hatten wir sehr viele Ideen zu Spielekonzepten. Während der Ideenfindungsphase hatte jedes Projektmitglied die Möglichkeit eigene Ideen und Vorschläge einzubringen und auszuarbeiten. Die daraus resultierenden Ideen fassten wir zu drei verschiedenen Konzepten zusammen über die wir dann abstimmten.\\
In Abbildung \ref{fig:concepts} sind die drei Spielekonzepte in Stichpunkten zusammengefasst auf der Abstimmungstafel zu sehen. In Spielekonzept 1 (SK1) war die Idee, dass die Spieler erst, wenn Sie ihre Zeichnung fertig gestellt haben und auf einen Bestätigungsknopf gedrückt haben, die Vorhersagen der künstlichen Intelligenz angezeigt bekommen. Das zweite Spielekonzept (SK2) unterschied sich vom ersten insofern, dass die Vorhersagen der KI immer sichtbar sein sollten. Der Unterschied zwischen dem dritten und zweiten Konzept sollte sein, dass hier nur eine noch festzulegende Anzahl an Vorhersagen angezeigt werden sollten.\\
Zuletzt fiel das Abstimmungsergebnis für das Spielkonzept 2 mit vier Stimmen von acht. Die beiden anderen Konzepte bekamen jeweils zwei Stimmen. Es sollte sich aber später im Verlaufe des Projektes festellen, dass das Spielkonzept 3 besser geeignet wäre, woraufhin wir dieses Umsetzten. Da die Unterschiede eher klein sind, war das auch kein zu großes Problem.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth]{images/spielekonzept.jpg}
	\caption{Abstimmung über die Spielekonzepte}
	\label{fig:concepts}
\end{figure}

\subsection{Responsive Design}
Da wir unser Spiel für mobile Endgeräte und Smartphones im Speziellen konzipiert hatten, stellte sich die Problematik, dass das Spiel auf manche Bildschirmgrößen kaum passte und auf Anderen sehr viel leere Flächen zu sehen waren.\\
Um gewährleisten zu können, dass das Spiel immer optimal im Browser angezeigt wird benutzten wir \textit{Media Queries}. Mit \textit{Media Queries} in CSS kann das Layout angepasst werden für unterschiedliche Bildschirmausrichtungen und Bildschirmgrößen. Wir entschieden uns zwei sogenannte \textit{Breakpoints} zu implementieren. Einen bei einer Bildschirmbreite von mindestens 720px und einen zweiten bei mindestens 1200px. Ab diesen Punkten werden nun je automatisch andere CSS-Regeln angewandt.\\
Außerdem vereinbarten wir bereits in der Anfangsphase unseres Projektes, dass wir regelmäßige Tests durchführen wollten. Die Tests wurden auf unterschiedlichen Smartphonegrößen durchgeführt und hierbei sollte auch jedesmal das Layout mit überprüft werden.\\
Besonders schwierig war das Iphone SE mit einem sehr kleinen Display, aber letzten Endes haben wir es auch darauf geschafft den kompletten Spielbildschirm anzuzeigen.

\subsection{Fehlendes Know-How zu neuronalen Netzen}
Die größte Quelle der Verunsicherung in unserem Team zu Beginn der Arbeiten war, dass niemand von uns Erfahrungen mit neuronalen Netzwerken hatte. Prof. Dr. Jung brachte bereits beim ersten Projekttreffen das Framework \textit{TensorFlow} ins Gespräch. Um uns mit der Thematik bekannt zu machen vereinbarten wir, dass jedes Projektteammitglied das offizielle Tutorial von \textit{TensorFlow} bearbeiten sollte. \parencite{tensorFlowTutorial}\\
Nachdem jeder das Tutorial durchgearbeitet hatte entschieden wir, dass nur maximal die Hälfte des Teams sich noch mehr in die Thematik einarbeiten sollte. Alle Anderen würden sich um andere Themen wie die weiter Entwicklung der Spielidee, das User Interface Design und die Entwicklung der Webseite kümmern.\\
Außerdem bot auch Prof. Dr. Jung seine Hilfe an, in dem er uns eine kleine Einführung in die Thematik gab.

\subsection{Probleme und weitere Optimierungsmöglichkeiten beim neuronalen Netz}
